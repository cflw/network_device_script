...
class C接口表:
	...
	def fe行(self):
		for v行 in self.m字符串.split("\n"):
			...
			v接口 = 实现接口.f创建接口m6000(v接口s)
			v状态 = "up" in v协议s
			yield 北向接口.S接口表项(a接口 = v接口, a状态 = v状态, a描述 = v描述s.strip())
#===============================================================================
# 地址解析表
#===============================================================================
class C地址解析表:
	...
	def fe行(self):
		for v行 in self.m字符串.split("\n"):
			...
			v接口 = 实现接口.f创建接口m6000(v子接口s)
			v时, v分, v秒 = (int(x) for x in v寿命s.split(":"))
			v总秒 = v时 * 3600 + v分 * 60 + v秒
			yield 信息.S地址解析表项(a网络地址 = v网络地址, a物理地址 = v物理地址, a接口 = v接口, a寿命 = v总秒)