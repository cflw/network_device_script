...
class C路由(南向路由协议.I进程配置):
	...
	def f模式_地址族(self, *a地址族, a操作 = 操作.E操作.e设置):
		...
		if v地址族类型 == 北向路由协议.E地址族.e单播4:
			return C单播地址族4(self)
		elif v地址族类型 == 北向路由协议.E地址族.e虚拟路由4:
			return C虚拟路由地址族4(self, a地址族[1])
		elif v地址族类型 == 北向路由协议.E地址族.e虚专网4:
			return C虚专网地址族4(self)
...
class C地址族(南向路由协议.I地址族配置):
	...
	def fg进入命令(self):
		return 命令.C命令("address-family") + self.fg模式参数()
	def fg退出命令(self):
...
class C单播地址族4(C地址族):
	def __init__(self, a):
		C地址族.__init__(self, a, "ipv4", "unicast")
	def f模式_对等体(self, a对等体, a操作 = 操作.E操作.e设置):
...
class C地址族对等体4(C对等体):
	...
	def fs远端自治系统号(self, a自治系统号, a操作 = 操作.E操作.e设置):
		...
		self.f执行当前模式命令(v命令)
class C虚拟路由地址族4(C地址族):
	def __init__(self, a, a虚拟路由转发):
		C地址族.__init__(self, a, "ipv4", "vrf", a虚拟路由转发)
class C虚专网地址族4(C地址族):
	def __init__(self, a):
		C地址族.__init__(self, a, "vpnv4")
