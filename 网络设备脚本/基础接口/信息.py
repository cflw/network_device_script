...
def f解析链路类型(a类型: str):
	...
	elif v类型.isdigit():	#思科show interface status中,access口会显示成vlan号
		return 虚拟局域网.E链路类型.e接入
	else:
...
def f解析链路聚合协议(a协议: str):
	from . import 链路聚合
	return 链路聚合.ca链路聚合协议.get(a协议, None)
#===============================================================================
# 解析-网络层
#===============================================================================
ca地址解析协议类型 = {
...
def f解析网络地址6(a地址: str):
	v地址 = a地址.lower()
	if "unassigned" in v地址:	#华三设备接口没地址会显示Unassigned
		return None
	if "--" in v地址:	#还没见到什么设备没地址显示成--
		return None
	if "/" in v地址:
		return 地址.S网络地址6.fc地址前缀长度字符串(v地址)
	else:
		return 地址.S网络地址6.fc主机地址字符串(v地址)