...
from ..基础接口 import 开放最短路径优先 as 北向路由协议
from ..命令行接口 import 命令
from ..命令行接口 import 模式
from ..命令行接口 import 路由 as 南向路由
from ..命令行接口 import 接口 as 南向接口
from ..命令行接口 import 开放最短路径优先 as 南向路由协议
from .常量 import *
...
class C进程配置(南向路由协议.I进程配置):
	...
	def fs通告接口(self, a接口, a区域号, a操作 = 操作.E操作.e设置):
		v接口 = self.f模式_接口(a接口)
		v接口.fs通告接口(a区域号, a操作)
class C区域4(南向路由协议.I区域配置, 模式.C同级模式):
	def __init__(self, a, a进程号, a区域号):
		南向路由协议.I区域配置.__init__(self, a, a进程号, a区域号)
...
class C接口4(南向路由协议.I接口配置):
	...
	def fs网络类型(self, a类型):
		raise NotImplementedError()
class C虚链路4(南向路由协议.I虚链路配置, 模式.C同级模式):
	def __init__(self, a, a进程号, a区域号, a对端):
		南向路由协议.I虚链路配置.__init__(self, a, a进程号, a区域号, a对端)
#===============================================================================
# 显示
#===============================================================================
ca邻居状态 = {
	"DOWN": 北向路由协议.E邻居状态.e关闭,
	"ATTEMP": 北向路由协议.E邻居状态.e尝试,
	"INIT": 北向路由协议.E邻居状态.e初始,
	"TWO-WAY": 北向路由协议.E邻居状态.e双向,
	"EXSTART": 北向路由协议.E邻居状态.e预启动,
	"EXCHANGE": 北向路由协议.E邻居状态.e交换,
	"FULL": 北向路由协议.E邻居状态.e完成
}
ca选举状态 = {
	"-": 北向路由协议.E选举状态.e无,
	"DR": 北向路由协议.E选举状态.e指定,
	"BDR": 北向路由协议.E选举状态.e备用,
	"DR other": 北向路由协议.E选举状态.e非指定
}
class C邻居表4:
	c邻居开始 = 0
	c优先级开始 = 16
	...
	def fe行(self):
		for v行 in 字符串.fe分割(self.m字符串, "\n"):
			if v行.count(".") != 6:	#2个地址共6个点
				continue
			v邻居s, v优先级s, v状态s, v死亡s, v地址s, v接口s = 字符串.fe按位置分割(v行, *C邻居表4.ca列开始)
			v邻居 = 地址.S网络地址4.fc主机地址字符串(v邻居s)
			v优先级 = int(v优先级s)
			v状态分割 = v状态s.split("/")
			v邻居状态 = ca邻居状态[v状态分割[0]]
			v选举状态 = ca选举状态[v状态分割[1]]
			v死亡时间分割 = v死亡s.split(":")
			v死亡时间 = datetime.timedelta(hours = int(v死亡时间分割[0]), minutes = int(v死亡时间分割[1]), seconds = int(v死亡时间分割[2]))
			v对端地址 = 地址.S网络地址4.fc主机地址字符串(v地址s)
			v接口 = 实现接口.f创建接口(v接口s)
			yield 北向路由协议.S邻居表项(a邻居标识 = v邻居, a优先级 = v优先级, a邻居状态 = v邻居状态, a选举状态 = v选举状态, a死亡时间 = v死亡时间, a对端地址 = v对端地址, a接口 = v接口)
