...
class C全局配置(全局配置.I全局配置模式):
	...
	def f模式_访问控制列表(self, a名称, a类型 = None, a操作 = 操作.E操作.e设置):
		from ..基础接口 import 访问控制列表 as 接口列表
		from ..网络设备 import 通用_访问控制列表 as 通用列表
		from . import 访问控制列表 as 实现列表
		v名称, v类型 = 通用列表.f解析名称和类型(a名称, a类型, 实现列表.C助手)
		#创建访问控制列表对象
		if v类型 == 接口列表.E访问控制列表类型.e标准4:
			实现列表.fi标准范围(v名称)
			v模式 = 实现列表.C标准4(self, v名称)
		elif v类型 == 接口列表.E访问控制列表类型.e扩展4:
			实现列表.fi扩展范围(v名称)
			v模式 = 实现列表.C扩展4(self, v名称)
		elif v类型 in (接口列表.E访问控制列表类型.e标准6, 接口列表.E访问控制列表类型.e扩展6):
