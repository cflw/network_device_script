...
class C登录(设备.I登录配置模式):
	...
	def f显示_当前模式配置(self):
		...
		return self.m配置
	#设置
	def fs认证方式(self, a认证方式 = None, a操作 = 操作.E操作.e设置):
		v命令 = 命令.C命令()
		v命令 += ca登录认证方式[a认证方式]
		self.f执行当前模式命令(v命令)
	def fs登录协议(self, a登录协议 = 设备.E登录协议.e全部, a操作 = 操作.E操作.e设置):
		...
		if a操作 == 操作.E操作.e重置:
			...
			if v列表:
				...
				v命令 += "none"
		self.f执行当前模式命令(v命令)
	def fs访问控制列表(self, a访问列表 = "", a操作 = 操作.E操作.e设置):
		v命令 = 命令.C命令("access-class %s in" % (a访问列表, ))
		if a操作 == 操作.E操作.e删除:
			v命令.f前面添加(c不)
		self.f执行当前模式命令(v命令)
	def fs登录超时(self, a秒 = 30, a操作 = 操作.E操作.e设置):
		v命令 = 命令.C命令("timeout login response")
		if a操作 == 操作.E操作.e删除:
			v命令.f前面添加(c不)
		else:
			v命令 += a秒
		self.f执行当前模式命令(v命令)
	def fs操作超时(self, a秒 = 600, a操作 = 操作.E操作.e设置):
