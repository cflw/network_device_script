...
def f解析路由条目(a元素):	#div
	...
	return {
		数据表.E字段.e目的路由类型: 路由.E路由类型.e静态,
		数据表.E字段.e目的网络号: 地址.S网络地址4.fc地址前缀长度(v网络号s, int(v掩码前缀s)),
		数据表.E字段.e目的下一跳: 地址.S网络地址4.fc地址前缀长度(v网关地址s, 32),
		数据表.E字段.e目的度量值: int(v权重s),
	}
class C静态路由_ad705(静态路由.I静态路由配置):
	"""适用于: ad 7.0.8"""
	...
	def __init__(self, a设备):
		self.m设备 = a设备
	def f切换到当前模式(self):
		self.m设备.f切换模式(模式.C模式_ad705.c网络部署_静态路由)
	def f显示_路由表(self):
		self.f切换到当前模式()
		time.sleep(1)	#表格会一直变化,元素会失效
		w表格 = self.m设备.f查找(C静态路由_ad705.c表格路径)
		def fe行():
