...
class F创建接口_ev6(北向接口.F创建接口):
	def f分隔符(self):	#分隔符和范围都是横杠, 无法解析范围
		return "-"
class F生成接口_ev6(北向接口.F生成接口):
	def f分隔符(self):
		return "-"
	def f生成范围(self, a范围):
		return f"{a范围.start} - {a范围.stop - 1}"
f生成接口_ev6 = F生成接口_ev6(ca接口名称ev6)
f创建接口_ev6 = F创建接口_ev6(ca接口名称ev6, f生成接口_ev6)
#接口
