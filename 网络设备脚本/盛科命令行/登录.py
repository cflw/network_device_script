...
# 模式
#===============================================================================
class C登录显示_ev6(北向登录.I登录显示, 模式.I显示模式):
	"""适用于: 浪潮CN61108PC-V-H(v6.6.6.R1.23)"""
	def __init__(self, a, a登录方式, a范围: range):
	...
	def fg操作超时(self):
		raise NotImplementedError()
class C登录配置_ev6(北向登录.I登录配置, 模式.I模式, C登录显示_ev6):
	"""适用于: 浪潮CN61108PC-V-H(v6.6.6.R1.23)"""
	def __init__(self, a, a登录方式, a范围: range):
		C登录显示_ev6.__init__(self, a, a登录方式, a范围)
		模式.I模式.__init__(self, a)
	def fg进入命令(self):
		"""命令: line 登录方式 数字 [数字]"""
		v命令 = 命令.C命令("line")
		v命令 += ca登录方式[self.m登录方式]
		v命令 += self.m范围.start
		if self.m登录方式 == 北向登录.E登录方式.e虚拟终端:
			v命令 += self.m范围.stop
		return v命令
	...
	def fs登录协议(self, a登录协议, a操作 = 操作.E操作.e设置):
		"""命令: transport input 协议"""
		v协议 = ca登录协议[a登录协议]
		v命令 = f"transport input {v协议}"
		self.f执行当前模式命令(v命令)
	def fs访问列表(self, a访问列表, a操作 = 操作.E操作.e设置):
		"""命令: access-class 访问列表 in"""
		v命令 = f"access-class {a访问列表} in"
		self.f执行当前模式命令(v命令)
	def fs登录超时(self, a秒, a操作 = 操作.E操作.e设置):
		pass	#无命令
	def fs操作超时(self, a秒, a操作 = 操作.E操作.e设置):
		"""命令: exec-timeout 分钟"""
		v命令 = 命令.C命令("exec-timeout")
		v命令 += math.ceil(a秒 / 60)
		self.f执行当前模式命令(v命令)
