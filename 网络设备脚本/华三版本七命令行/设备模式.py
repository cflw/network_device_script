...
from . import 系统信息
from . import 硬件信息
class C设备显示v7(设备模式.I设备显示, 模式.I显示模式):
...
class C设备显示_模拟v7_1(设备模式.I设备显示, 模式.I显示模式):
	...
	def f显示_风扇(self):
		v输出 = self.m设备.f执行显示命令("display fan")
		return 硬件信息.f简单风扇信息v7(v输出)
	def f显示_电源(self):
		v输出 = self.m设备.f执行显示命令("display power")
		return 硬件信息.f简单电源信息v7(v输出)
	def f显示_温度(self):
		v输出 = self.m设备.f执行显示命令("display environment")
